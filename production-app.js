...
import {
  getAuth, onAuthStateChanged, signOut, deleteUser, signInWithPopup, GoogleAuthProvider
...
// Global user variable (if needed elsewhere, otherwise remove)
let userGlobal = null;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // PIN authentication UI elements
  ...
  // Auth flow with PIN authentication
  document.body.style.visibility = "hidden"; // Hide body until auth state is determined
  onAuthStateChanged(auth, async (user) => {
    document.body.style.visibility = ""; // Show body once auth state is known
    if (!user) {
      if (authOverlay) authOverlay.style.display = "flex";
      userGlobal = null;
      // Ensure PIN modals are hidden if user logs out
    ...
    if (!userSnap.exists()) {
      // First-time user: generate a unique 8-digit PIN
      ...
      if (initialPinDisplay) initialPinDisplay.textContent = pin;
      if (initialPinModal) initialPinModal.classList.remove("hidden");
      if (authOverlay) authOverlay.style.display = "flex"; // Keep overlay during PIN display

      if (initialPinConfirmBtn) {
      ...
      // Returning user: prompt for PIN
      if (pinEntryModal) pinEntryModal.classList.remove("hidden");
      if (authOverlay) authOverlay.style.display = "flex"; // Keep overlay until PIN is entered
      ...
      if (pinEntryForm) {
        pinEntryForm.addEventListener("submit", async (e) => {
          ...
          if (!entered || !/^\d{8}$/.test(entered)) {
            if (pinErrorMessage) pinErrorMessage.textContent = "PIN must be 8 digits.";
            return;
          }

          const enteredHash = await hashPIN(entered);
          if (enteredHash === userSnap.data().pinHash) {
            pinEntryModal?.classList.add("hidden");
  ...
  // Optional close buttons for modals
  document.getElementById("closeInitialPinModalBtn")?.addEventListener("click", () => {
    initialPinModal?.classList.add("hidden");
    // If the initial PIN modal is closed, and user hasn't confirmed, they are effectively not signed in for app use
    if (!userGlobal) authOverlay.style.display = "flex"; // Re-show overlay if user hasn't fully logged in
  });
  document.getElementById("closePinEntryModalBtn")?.addEventListener("click", () => {
    pinEntryModal?.classList.add("hidden");
    // If PIN entry modal is closed without success, keep overlay
    if (!userGlobal) authOverlay.style.display = "flex"; // Re-show overlay if user hasn't fully logged in
  });
  ...
  // DOMContentLoaded listener is already wrapped, so no need for this specific one
  restoreSellForm();
