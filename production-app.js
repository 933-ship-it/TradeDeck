...
// --- Email Notification via EmailJS (now via backend) ---
async function sendSaleEmail({ to_email, product_title, buyer_name }) {
  try {
    ...
    if (response.ok) {
      ...
      alert('Failed to send email notification to seller.');
    }
  } catch (error) {
    console.error('Error sending email:', error);
    alert('An error occurred while sending email notification.');
  }
}
        ...
        window.paypal.Buttons({
          ...
          onApprove: async function(data, actions) {
            ...
            try {
              ...
              if (response.ok) {
                alert('Payment verified by ' + result.details.payer.name.given_name + '!');
                ...
                sendSaleEmail({
                  to_email: product.paypalEmail || 'Not Provided', // Seller's email
                  product_title: product.title || 'Unknown',
                  buyer_name: (result.details.payer && result.details.payer.name && result.details.payer.name.given_name) ? result.details.payer.name.given_name : 'Unknown'
                });
              } else {
